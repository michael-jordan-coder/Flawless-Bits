...
const buildCriteria = (
  value: string,
  { minLength, requireUppercase, requireNumber, requireSymbol }: CriteriaConfig
): Criterion[] => {
...
export default function PasswordStrength({
  ...
  const barTransition: Transition = prefersReduced ? { duration: 0 } : { duration: 0.3, ease: 'easeOut' };
  const rowSpring: Transition = prefersReduced ? { duration: 0 } : { type: 'spring', stiffness: 520, damping: 24 };
