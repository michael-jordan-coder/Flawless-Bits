...
export default function FocusCarousel({
  ...
  return (
    <div {...rest} className={join('relative mx-auto w-full max-w-[760px]', className)} style={style}>
      <div
        ref={scrollerRef}
        className="box-border flex h-[var(--focus-carousel-height)] items-center gap-[var(--focus-carousel-gap)] overflow-x-auto overflow-y-hidden px-[var(--focus-carousel-peek)] py-7 [-ms-overflow-style:none] [overscroll-behavior-x:contain] [scroll-behavior:smooth] [scroll-snap-type:x_mandatory] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden motion-reduce:[scroll-behavior:auto]"
      >
        {items.map((item, i) => (
          <div
            key={item.id ?? i}
            ...
            className="flex h-full shrink-0 basis-[var(--focus-carousel-card)] items-center [scroll-snap-align:center]"
          >
            <div
              ref={el => {
                frameRefs.current[i] = el;
              }}
              className="relative h-full w-full overflow-hidden rounded-[20px] bg-[#141417] [box-shadow:0_22px_48px_-24px_rgba(0,0,0,0.7)] [will-change:filter,opacity] motion-reduce:!opacity-100 motion-reduce:!filter-none"
            >
              <img
