...
export default function ArcCarousel({
  ...
  return (
    <div {...rest} className={join('arc-carousel', className)} style={style}>
      <div ref={scrollerRef} className="arc-carousel-scroller">
        {items.map((item, i) => (
          <div
            key={item.id ?? i}
            ref={el => {
            ...
            className="arc-carousel-slot"
          >
            <div
              ref={el => {
                frameRefs.current[i] = el;
              }}
              className="arc-carousel-frame"
            >
              <img
                className="arc-carousel-image"
                src={item.image}
                alt={item.title ?? ''}
                draggable={false}
                loading="lazy"
              />
              <div className="arc-carousel-scrim" aria-hidden="true" />
              {(item.title || item.subtitle) && (
                <div className="arc-carousel-caption">
                  {item.title && <span className="arc-carousel-title">{item.title}</span>}
                  {item.subtitle && <span className="arc-carousel-subtitle">{item.subtitle}</span>}
                </div>
              )}
            </div>
          </div>
