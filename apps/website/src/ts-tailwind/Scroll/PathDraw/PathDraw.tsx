...
import { motion, useScroll, useMotionTemplate, useReducedMotion, type MotionValue } from 'motion/react';

const join = (...classes: Array<string | false | undefined>) => classes.filter(Boolean).join(' ');

type ShapeName = 'wave' | 'spiral' | 'route';
...
function PathHeadDot({ d, progress, color, dotRadius, reduce }: PathHeadDotProps) {
...
export interface PathDrawProps extends HTMLAttributes<HTMLDivElement> {
  shape?: ShapeName;
  d?: string;
  color?: string;
  strokeWidth?: number;
